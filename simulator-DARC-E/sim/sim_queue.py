...
def insert_with_key(lst, element, key_func):
    if len(lst) == 0:
        lst.append(element)
        return False
    else:    
        index = bisect.bisect_right([key_func(x) for x in lst], key_func(element))
        if index == len(lst):
            lst.append(element)
            return False
        else:
...
class Queue:
    ...
    def enqueue(self, task, set_original=False,job_id_ordered = False, requeued=False, stolen=False, flag_steal=False,
        ...
        if stolen:
            ...
            if job_id_ordered == False:
                self.queue.append(task)
            else:
                #优先完成作业序号在前的作业的任务
                #index = bisect.bisect_right(self.queue, task, key = lambda x: x.job_id)
                #self.queue.insert(index, task)
                if insert_with_key(self.queue, task , key_func = lambda x: x.job_id) == True:
                    return True
    ...
    def current_delay(self, second=False):
        """当前队列延迟，默认参照物是队列头部，second参数为true则为第二个任务
        """
    ...
    def second(self):
        ...
        return self.queue[len(self.queue)//2] if len(self.queue) >= 2 else None
