...
class Thread:
    ...
    def schedule(self, time_increment=1, Requeue=False):
        ...
        if self.is_busy():
            ...
            if isinstance(self.current_task, WorkStealTask):
                if len(self.state.queues[self.queue.id].queue) > 1:
                    self.current_task = None
                    self.schedule()
        ...
        # 首先检查本地队列
        elif self.work_search_state == WorkSearchState.LOCAL_QUEUE_FIRST_CHECK or len(self.state.queues[self.queue.id].queue) > 0:
        ...
        # 尝试work stealing
        elif self.work_search_state == WorkSearchState.WORK_STEAL_CHECK and self.queue.id > 0:
