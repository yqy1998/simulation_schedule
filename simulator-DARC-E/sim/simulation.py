...
#        while self.state.any_incomplete() and \
        while(self.config.sim_duration is None or self.state.timer.get_time() < self.config.sim_duration):
    ...
    def save_stats(self):
        ...
        # Write task information(这里面时读取所有的任务，如果用服务器集群，则有的任务)-------需要改，可以只写完成的任务
...
if __name__ == "__main__":
    ...
    # #记录task的完成情况
    # fileName = run_name
    # data = sim.state.tasks
    # workbook = xw.Workbook(fileName)  # 创建工作簿
    # worksheet1 = workbook.add_worksheet("sheet1")  # 创建子表
    # worksheet1.activate()  # 激活表
    # title = ['task_id', 'type', 'task_generation_time','service_time','arrival_time','start_time','completion_time']  # 设置表头
    # worksheet1.write_row('A1', title)  # 从A1单元格开始写入表头
    # i = 2  # 从第二行开始写入数据
    # for j in range(len(data)):
    #     insertData = [data[j]
    #                   ["task_id"], data[j]["type"], data[j]["task_generation_time"],data[j]["service_time"],data[j]["arrival_time"],data[j]["start_time"],data[j]["completion_time"]]
    #     row = 'A' + str(i)
    #     worksheet1.write_row(row, insertData)
    #     i += 1
    # workbook.save("/home/yqy/simulator-DARC-E/task_record/")
    # workbook.close()  # 关闭表
    #记录配置文件
