...
class Task:
    ...
    def __init__(self, time, arrival_time, config, state, type_def,job_id,task_generation_time,tasks_num_of_single_job):
        self.source_core = None
        self.service_time = time

        self.time_left = time
        self.complete = False
        #用于跟踪任务在网络中传输后到达服务器的时间
        ...
        self.job_id = job_id
        #用于记录任务生成时间
        self.task_generation_time = task_generation_time
        #一个作业下的任务总数
        self.tasks_num_of_single_job = tasks_num_of_single_job
...
#跟着get_stat_headers一起改
    def get_stats(self):
        ...
        stats += [self.job_id,self.tasks_num_of_single_job,self.task_generation_time,self.completion_time]
        stats = [str(x) for x in stats]
        return stats

#需要修改一处（加上task的ID(所属作业),类型，以及生成时间）
    @staticmethod
    def get_stat_headers(config):
        ...
        headers += ["job_id","tasks_num_of_single_job","task_generation_time","completion_time"]
        return headers
...
class AbstractWorkStealTask(Task):
    ...
    def __init__(self, thread, initial_time, config, state):
        ##################改变task的值，每次初始化都要改变以下函数的值
        super().__init__(initial_time, state.timer.get_time(), config, state, type_def="micro",job_id = -1,task_generation_time = state.timer.get_time(),tasks_num_of_single_job = 1)
        self.thread = thread
...
class ReallocationTask(Task):
    ...
    def __init__(self, thread, config, state):
        super().__init__(config.ALLOCATION_TIME, state.timer.get_time(), config, state, type_def="micro",job_id = -1,task_generation_time = state.timer.get_time(),tasks_num_of_single_job = 1)
        self.is_productive = False
        self.thread = thread
...
class WorkSearchSpin(Task): # TODO: Check the preemption for double-counting
    ...
    def __init__(self, thread, config, state):
        super().__init__(config.MINIMUM_WORK_SEARCH_TIME, state.timer.get_time(), config, state, type_def="micro",job_id = -1,task_generation_time = state.timer.get_time(),tasks_num_of_single_job = 1)
        self.is_productive = False
        self.thread = thread
        self.preempted = False
...
class QueueCheckTask(Task):
    ...
    def __init__(self, thread, config, state, return_to_ws_task=None):
        super().__init__(config.LOCAL_QUEUE_CHECK_TIME, state.timer.get_time(), config, state, type_def="micro",job_id = -1,task_generation_time = state.timer.get_time(),tasks_num_of_single_job = 1)
        self.thread = thread
...
class WorkStealTask(AbstractWorkStealTask):
    ...
    def work_search_walk(self):
        ...
        # # Select a random thread to steal from then walk through all
        # if len(self.state.queues[0].queue) > 1:
        #     self.start_work_steal_check(self.state.queues[0])
        # elif len(self.state.queues[1].queue) > 1 and self.current_type_level > 1:
        #     self.start_work_steal_check(self.state.queues[1])
        self.search_index += 1
