...
#输入格式： python3 simulation_cluster_test.py 8 config.json  data_23_7_4
    if "-d" in sys.argv:
    ...
    #读取json文件配置，服务器集群每个服务器的此配置一样（包括task和simulation时长）
    if os.path.isfile(sys.argv[2]):
        cfg_json = open(sys.argv[2], "r")
        cfg = json.load(cfg_json, object_hook=SimConfig.decode_object)
        #注释掉，不同服务器的run_name不一样
        #cfg.name = run_name
        cfg_json.close()
