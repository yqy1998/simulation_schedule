...
class SimulationState:
    ...
    def current_buffer_cores(self, check_work_available=False):
        ...
        for thread in self.threads:
            if not thread.is_productive() and not thread.work_search_state == WorkSearchState.PARKED and \
                    ...
                    not(self.config.enqueue_choice and thread.queue.awaiting_enqueue) and thread.id != 0 and thread.id != 2:
    ...
    def initialize_state(self, config, tasks):
        ...
        for i in range(config.num_threads):
            if config.num_threads % 2 == 1 and config.num_threads-1 == i:
                ...
                self.threads[i].sibling = self.threads[i - 1]
        #设置任务tasks[[job_id,num_of_single_job,task_generation_time,arrival_time,service_time]]
        i = 0
        while i < len(tasks):
            type_def = def_type(tasks[i][4])
            self.tasks.append(Task(tasks[i][4], tasks[i][3], config, self,type_def,tasks[i][0],tasks[i][2],tasks[i][1]))
            i = i+1
